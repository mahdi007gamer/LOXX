...
const ChatAnimation = () => {
 ...
 <span className="text-[10px] text-neon-pink font-black ">1,248 ONLINE</span>
 </div>
...
const ProfileAnimation = ({ name }: { name: string }) => {
 ...
 <span className="text-[#ffaa00] text-[9px] font-black mt-0.5 flex items-center gap-1 uppercase">
...
const EnvelopeIntro = ({ onOpen, streamerName }: { onOpen: () => void, streamerName: string }) => {
 ...
 <h2 className="text-3xl lg:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 text-center mb-2 uppercase" dir="ltr">
 SECRET DOSSIER
 </h2>
...
const FloatingAudioPlayer = ({ src, streamerName, isOpened }: { src: string, streamerName: string, isOpened: boolean }) => {
 ...
 <span className="text-white font-black text-sm mb-0.5" dir="ltr">
 پیام صوتی دعوت
 </span>
 <span className="text-neon-pink/80 text-[11px] font-bold uppercase" dir="ltr">
 {`FROM @${streamerName}`}
 </span>
 </div>
 </div>
...
const StreamerProposalPage = ({ overrideName }: StreamerProposalPageProps = {}) => {
 ...
 <p>
 با بررسی دقیق استریم‌های حرفه‌ای تو و پتانسیل بی‌نظیر کامیونیتی بزرگت، پکیج ویژه و اختصاصی <span className="text-neon-pink font-black uppercase ">Elite Streamer</span> رو برای پیوستن تو به این اکوسیستم طراحی کردیم تا دغدغه‌های مدیریت لابی‌ها و پایداری شبکه‌ت رو به صفر برسونیم.
 </p>
 </div>
 </motion.section>
 ...
 <h4 className="text-xl font-black text-white mb-3 relative z-10">پروفایل هوشمند و <span className="uppercase font-sans">Elite</span></h4>
 <p className="text-gray-400 font-medium leading-relaxed text-[14px] relative z-10">
 ...
 <div>
 <span className="text-[11px] text-gray-500 font-bold uppercase block mb-1">نام و نشان استریمر</span>
 <span className="text-sm font-bold text-gray-200">{proposalStatus.fullName}</span>
 </div>
 <div>
 <span className="text-[11px] text-gray-400 font-bold uppercase block mb-1">شماره تماس جهت هماهنگی</span>
 <span dir="ltr" className="text-sm font-bold text-gray-200 block text-right">{proposalStatus.phone}</span>
 </div>
 <div>
 <span className="text-[11px] text-gray-400 font-bold uppercase block mb-1">تمایل به اسپانسرشیپ رسمی</span>
 ...
 <div>
 <h5 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 mb-3 ">لوکس؛ فراتر از یک اتصال.</h5>
