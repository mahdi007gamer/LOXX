...
export const ProfilePage = () => {
 ...
 <div className={cn(
 "rounded-full px-4 py-1 text-[10px] font-black uppercase border shadow-xl shrink-0",
 ...
 <span className="text-[10px] text-gray-500 font-bold uppercase flex items-center gap-1.5 whitespace-nowrap">
 <Clock size={12} /> عضویت از {profileData?.stats?.daysSinceJoin || 0} روز پیش
 </span>
 <div className="h-1 w-1 rounded-full bg-gray-700" />
 {profileData?.isVerified ? (
 <span className="text-[10px] text-green-500 font-bold uppercase flex items-center gap-1.5 whitespace-nowrap">
 <CheckCircle2 size={12} /> تایید شده
 </span>
 ) : (
 <span className="text-[10px] text-amber-500 font-bold uppercase flex items-center gap-1.5 whitespace-nowrap">
 <Shield size={12} /> تایید نشده
 </span>
 )}
 </div>
 <p className="mt-4 text-gray-400 text-sm max-w-2xl leading-relaxed line-clamp-2">{profileData?.bio || "بایوگرافی هنوز تنظیم نشده است"}</p>
 </div>

 <div className="flex items-center gap-3 self-center sm:self-end mt-4 sm:mt-0">
 <GlowButton variant="blue" className="px-8 h-12 gap-2 !rounded-2xl font-black text-xs uppercase" onClick={() => setIsEditModalOpen(true)}>
 <Edit2 size={16} />
 ...
 <p className="text-[9px] text-gray-500 font-black uppercase mb-1 ">روز عضویت</p>
 <p className="text-2xl font-black text-white ">{profileData?.stats?.daysSinceJoin || 0}</p>
 </div>
 ...
 <p className="text-[9px] text-gray-500 font-black uppercase mb-1 ">لابی‌های شرکت کرده</p>
 <p className="text-2xl font-black text-white ">{profileData?.stats?.lobbiesJoined || 0}</p>
 </div>
 ...
 <p className="text-[9px] text-gray-500 font-black uppercase mb-1 ">تعداد دوستان</p>
 <p className="text-2xl font-black text-white ">{profileData?.stats?.friendsCount || 0}</p>
 </div>
 ...
 <p className="text-[9px] text-gray-500 font-black uppercase mb-1 ">لابی‌های ساخته شده</p>
 <p className="text-2xl font-black text-white ">{profileData?.stats?.lobbiesCreated || 0}</p>
 </div>
 </div>
 </NeonCard>
 ...
 <p className="text-[10px] text-gray-500 font-black uppercase ">سطح کاربری (Level)</p>
 <p className="text-xl font-black text-neon-blue ">Lv. {profileData?.level || 1}</p>
 </div>
 
 <div className="space-y-3">
 <p className="text-[10px] text-gray-500 font-black uppercase mr-2">نشان‌های کسب شده</p>
 ...
 <p className="text-[10px] text-gray-600 ">هنوز نشانی کسب نکرده‌اید</p>
 )}
 </div>
 <p className="text-[9px] text-gray-600 mt-2">* نشان‌های دور آبی در مینی‌پروفایل شما نمایش داده می‌شوند.</p>
 </div>
 </div>
 </NeonCard>
 ...
 <h3 className="flex items-center gap-3 font-black text-white uppercase text-lg mb-4">
 ...
 <p className="text-xs text-green-400/80 mb-1 leading-relaxed ">
 ...
 <p className="text-[10px] text-neon-blue font-black uppercase mb-1 ">اطلاعیه</p>
 <p className="text-xs text-gray-400 leading-relaxed ">تنظیمات مینی‌پروفایل شما (رنگ‌ها، افکت‌ها و فریم‌ها) فقط برای اعضای VIP و PLUS در دسترس است. هم‌اکنون حساب خود را ارتقا دهید.</p>
 </div>
 </NeonCard>
 </div>
 </div>
 ...
 <div>
 <label className="text-[10px] font-black text-gray-500 uppercase mb-2 block ">نام نمایشی (Display Name)</label>
 <input type="text" value={editForm.displayName} onChange={e => setEditForm({...editForm, displayName: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-sm text-white focus:border-neon-blue focus:outline-none font-bold " />
 </div>
 <div>
 <label className="text-[10px] font-black text-gray-500 uppercase mb-2 block ">آیدی (Username)</label>
 <input type="text" value={user?.username} disabled className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-sm text-gray-600 font-bold " />
 </div>
 </div>
 
 <div>
 <label className="text-[10px] font-black text-gray-500 uppercase mb-2 block ">بایوگرافی پروفایل (Bio)</label>
 <textarea value={editForm.bio} onChange={e => setEditForm({...editForm, bio: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-sm text-white focus:border-neon-blue focus:outline-none font-bold h-32 resize-none" placeholder="کمی درباره خودت بنویس..." />
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
 <div className="space-y-4">
 <label className="text-[10px] font-black text-gray-500 uppercase mb-2 block ">تصویر پروفایل (Avatar)</label>
 ...
 <div className="space-y-4">
 <label className="text-[10px] font-black text-gray-500 uppercase mb-2 block ">بنر پروفایل (Banner)</label>
 <div className="space-y-3">
 ...
 <button onClick={() => setIsEditModalOpen(false)} className="h-12 px-8 text-xs font-black text-gray-500 hover:text-white uppercase ">انصراف</button>
 <GlowButton className="h-12 px-12 !rounded-2xl font-black text-xs uppercase " onClick={handleUpdateProfile}>ذخیره تغییرات نهایی</GlowButton>
 </div>
 </div>
 </Modal>
 </div>
