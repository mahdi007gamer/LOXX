...
export const SmartImage: React.FC<SmartImageProps> = ({ 
      ...
      if (fullUrl.includes("/api/v1/upload/")) {
        ...
        try {
          ...
          if (isMounted) {
            ...
            if (status === 404) {
              console.warn(`SmartImage: Image asset not found (404) at path ${urlPath}`);
            } else {
              console.warn(`SmartImage: Skipping API blob load for ${urlPath} (${err?.message || err})`);
            }
            handleImageError();
          }
        }
  ...
  }, [src, fallbackIndex, memoizedFallbacks, handleImageError]);
