...
// Add imports for useEffect and ticket list
...
if (!code.includes("myTickets")) {
  ...
  const [selectedTicket, setSelectedTicket] = useState<any>(null); // For modal
...
// Check for active ticket pending state.
// user cannot submit a ticket if one is pending.
const ticketBox = `
                        ...
                        <div key={ticket.id} className="bg-dark-card p-4 rounded-xl border border-white/5 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                {ticket.status === 'PENDING' && (
                                   <span className="px-2 py-1 bg-orange-500/10 text-orange-400 text-[10px] font-bold rounded">
                                ...
                                {ticket.status === 'ACTIONED' && (
                                   <div className="flex items-center gap-2">
                                     ...
                                     <button 
                                        ...
                                        className="text-[10px] font-black text-neon-blue bg-neon-blue/10 px-3 py-1 rounded hover:bg-neon-blue/20 transition-all"
                                     >
                                        {isRtl ? "مشاهده پاسخ" : "View"}
                                     </button>
                                   </div>
                                )}
...
if (!code.includes("Response Modal")) {
   ...
   // Wait, if it's the root component return, we can add it before the last </div>
   // Because wrap with <> in return statement which was just <div ...> </div>
