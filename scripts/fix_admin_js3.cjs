...
if (!code.includes("RESPOND_TICKET")) {
   code = code.replace(/\{report\.status === "PENDING" && \(\s*<div className="grid grid-cols-2 gap-2 mt-auto pt-3 border-t border-white\/5">/, 
       `{report.status === "PENDING" && (
         <div className="grid grid-cols-2 gap-2 mt-auto pt-3 border-t border-white/5">
             ${ticketActions}
       `);

   code = code.replace(/رد گزارش\s*<\/button>\s*<\/div>\s*\)\}/g, 
       `رد گزارش\n                          </button>\n          </>\n       )\n    }\n         </div>\n                  )}`);
}

fs.writeFileSync("src/pages/AdminPage.tsx", code);
console.log("AdminPage JS rewrite applied with RegEx");
