...
code = code.replace(/className=\{cn\(\n\s*"px-3 py-1 rounded-lg transition-all duration-150 border",/g, 'className={cn(\n  "px-3 py-1 rounded-lg transition-all duration-150 border w-[130px]",');

code = code.replace(/<span className="font-sans truncate max-w-\[120px\]">/g, '<span className="font-sans truncate flex-1">');

fs.writeFileSync('src/pages/DesktopOverlayWidget.tsx', code);
console.log("Fixed overlay alignment");
