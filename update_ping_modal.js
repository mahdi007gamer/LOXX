...
const modalRegex = /<Modal title=\{isRtl \? "پینگ بالا تشخیص داده شد" : "High Ping Detected"\}[\s\S]*?<\/Modal>/;

const replacement = `<Modal title={isRtl ? "پینگ بالا تشخیص داده شد" : "High Ping Detected"} onClose={() => setShowHighPingModal(false)} maxWidth="max-w-md">
      ...
      <div className="w-full bg-[#0a0a0f] p-4 rounded-xl border border-white/5 space-y-3 mt-2 text-right">
        <div className="flex justify-between items-center text-sm">
          ...
          <button 
            ...
            className={cn("px-3 py-1 font-bold rounded transition border", bypassSystemProxy ? "bg-gray-500/20 text-gray-400 border-gray-500/30 cursor-not-allowed" : "bg-emerald-500/20 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/30")}>
            {bypassSystemProxy ? 'فعال است' : 'فعال‌سازی سریع'}
          </button>
        </div>
        ...
        <div className="flex justify-between items-center text-sm">
          ...
          <button 
            disabled={appDnsProvider === "shecan"}
            onClick={() => {
              updateLauncherSettings({ appDnsProvider: "shecan" });
              toast.success("سرویس DNS روی شکن تنظیم شد!");
            }}
            className={cn("px-3 py-1 font-bold rounded transition border", appDnsProvider === "shecan" ? "bg-gray-500/20 text-gray-400 border-gray-500/30 cursor-not-allowed" : "bg-[#00e5ff]/20 text-[#00e5ff] hover:bg-[#00e5ff]/30 border-[#00e5ff]/30")}>
            {appDnsProvider === "shecan" ? 'تنظیم شده' : 'اعمال سرویس'}
          </button>
        </div>
      </div>
...
code = code.replace(modalRegex, replacement);
fs.writeFileSync('src/pages/LobbyRoomPage.tsx', code);
console.log("Updated high ping modal");
